    ...
    {       
            ...
            string runtime = null)
        {
            ...
            using (var input = typeof(Project).Assembly.GetManifestResourceStream(
                "DbContextMappingDump.Resources.EntityFrameworkCore.targets"))
            using (var output = File.OpenWrite(efTargetsPath))
            {
                // NB: Copy always in case it changes
                Reporter.WriteVerbose($"WritingFile(efTargetsPath)");
                input.CopyTo(output);
            }
            ...
            try
            {
                ...
                var res = Exe.Run("dotnet", args);
                if (res.exitCode != 0)
                {
                    throw new CommandException($"GetMetadataFailed");
                }
        ...
        public void Build()
        {
            ...
            if (res.exitCode != 0)
            {
                throw new CommandException($"BuildFailed");
            }
        }
    }
}
